...
import {NodeClient} from '../../../auth-session/client/node-client';
...
describe('node client', function () {
  ...
  it('should send the correct default headers', async function () {
    ...
    const decodedTelemetry = JSON.parse(Buffer.from(headers['auth-client'] as string, 'base64').toString('ascii'));
