...
import {ConfigParameters, getConfig} from '../../../auth-session';
import {EdgeClient} from '../../../auth-session/client/edge-client';
import {AuthRequest} from '../../../auth-session/http';
import {UserInfoError} from '../../../auth-session/utils/errors';
import version from '../../../version';
import {mockFetch} from '../../fixtures/fetch';
import {jwks, makeIdToken} from '../fixtures/cert';
import wellKnown from '../fixtures/well-known.json';

class TestReq extends AuthRequest<null> {
  constructor() {
    super(null);
  }
  getBody() {
...
describe('edge client', function () {
  ...
  it('should send the correct default headers', async function () {
    ...
    expect(headerProps).toContain('auth-client');

    const decodedTelemetry = JSON.parse(new TextDecoder().decode(jose.base64url.decode(headers['auth-client'])));

    expect(decodedTelemetry.name).toEqual('nextjs-auth');
    ...
    expect(headers['user-agent']).toEqual(`nextjs-auth/${pkg.version}`);
  ...
  it('should disable telemetry', async function () {
    ...
    expect(headerProps).not.toContain('auth-client');
    expect(userinfo.sub).toBe('foo');
  });
