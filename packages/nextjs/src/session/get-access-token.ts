...
import {AuthorizationParameters} from '../auth-session';
import {GetClient} from '../auth-session/client/abstract-client';
import {GetConfig} from '../config';
import {AuthNextRequestCookies} from '../http';
import {fromTokenEndpointResponse, get, Session, SessionCache, set} from '../session';
import {intersect, match} from '../utils/array';
import {AccessTokenError, AccessTokenErrorCode} from '../utils/errors';
import {getAuthReqRes} from './cache';
...
export interface AccessTokenRequest {
   ...
   * import { getAccessToken } from '@loopauth/nextjs';
   *
 ...
 * import { NextResponse } from 'next/server';
 * import { getAccessToken, withApiAuthRequired } from '@loopauth/nextjs';
 *
 ...
 * // app/my-api/route.js
 * import { NextResponse } from 'next/server';
 * import { getAccessToken } from '@loopauth/nextjs/edge'; // Note the /edge import
 *
 * export async function GET() {
 *   const { accessToken } = await getAccessToken();
 *   return NextResponse.json({ foo: 'bar' });
...
export default function accessTokenFactory(
  ...
  return async (reqOrOpts?, res?, accessTokenRequest?): Promise<GetAccessTokenResult> => {
    ...
    const config = await getConfig(req ? getAuthReqRes(req, res as any)[0] : new AuthNextRequestCookies());
