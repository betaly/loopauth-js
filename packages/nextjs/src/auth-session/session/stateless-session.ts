...
> extends AbstractSession<Session> {
  ...
  public async encrypt(payload: jose.JWTPayload, {iat, uat, exp}: Header, key: Uint8Array): Promise<string> {
    return new jose.EncryptJWT({...payload}).setProtectedHeader({alg, enc, uat, iat, exp}).encrypt(key);
  }

  async getSession(req: AuthRequestCookies): Promise<SessionPayload<Session> | undefined | null> {
