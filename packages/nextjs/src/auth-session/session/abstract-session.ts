...
import {AuthRequestCookies, AuthResponseCookies} from '../http';
...
export abstract class AbstractSession<Session> {
  protected getConfig: (req: AuthRequestCookies) => Config | Promise<Config>;

  constructor(getConfig: GetConfig) {
    this.getConfig = typeof getConfig === 'function' ? getConfig : () => getConfig;
  }

  abstract getSession(req: AuthRequestCookies): Promise<SessionPayload<Session> | undefined | null>;

  abstract setSession(
    req: AuthRequestCookies,
    res: AuthResponseCookies,
    session: Session,
    uat: number,
    iat: number,
    exp: number,
  ...
  public async read(req: AuthRequestCookies): Promise<[Session?, number?]> {
