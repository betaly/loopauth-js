...
import {AuthRequestCookies} from './http';
...
export interface Config {
  ...
  /**
   * Boolean value to enable Auth's logout feature.
   */
  auth0Logout?: boolean;
   ...
   * via the `Auth-Client` header. Defaults to `true`.
   */
...
export type GetConfig = Config | ((req: AuthRequestCookies) => Config | Promise<Config>);
