...
export class OAuthNodeClient extends AbstractClient {
  ...
  async callbackParams(req: AuthRequest) {
    const client = await this.getClient();
...
export const clientGetter = (telemetry: Telemetry): ((config: Config) => Promise<OAuthNodeClient>) => {
  let client: OAuthNodeClient;
  return async config => {
    if (!client) {
      client = new OAuthNodeClient(config, telemetry);
    }
    return client;
  };
};
