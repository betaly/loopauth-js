...
export class OAuthEdgeClient extends AbstractClient {
  ...
  async callbackParams(req: AuthRequest, expectedState: string) {
    const [as, client] = await this.getClient();
    const url =
  ...
  async generateRandomCodeVerifier() {
    return oauth.generateRandomCodeVerifier();
  }

  async generateRandomNonce() {
...
export const clientGetter = (telemetry: Telemetry): ((config: Config) => Promise<OAuthEdgeClient>) => {
  let client: OAuthEdgeClient;
  return async config => {
    if (!client) {
      client = new OAuthEdgeClient(config, telemetry);
    }
    return client;
  };
};
